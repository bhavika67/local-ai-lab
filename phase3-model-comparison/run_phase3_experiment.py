...
def score_quality(
    ...
    result["actual_sentiment"] = (
        model_output.get("sentiment")
    )

    result["actual_toxicity"] = (
        model_output.get("is_toxic")
    )
    ...
    result["actual_sarcasm"] = (
        model_output.get("contains_sarcasm")
    )

    result["actual_language"] = (
        model_output.get("language")
    )

    result["expected_sarcasm"] = (
    ...
    actual_sentiment = model_output.get(
        "sentiment"
    )
    ...
    actual_toxicity = model_output.get(
        "is_toxic"
    )

    result["expected_toxicity"] = (
        expected_toxicity
    )
...
def update_summary(
    ...
    failure_type = quality_result.get(
        "failure_type"
    )
...
def print_summary(summary):
    ...
    for model_name, temps in summary.items():
        ...
        for temperature, data in temps.items():
            ...
            sentiment_acc = (
                data["sentiment_correct"]
                / scoreable
            ) * 100

            toxicity_acc = (
                data["toxicity_correct"]
                / scoreable
            ) * 100
            ...
            sarcasm_acc = (
                data["sarcasm_correct"]
                / data["runs"]
            ) * 100

            language_acc = (
                data["language_correct"]
                / data["runs"]
            ) * 100

            avg_score = (
                sum(data["overall_scores"])
                /
                len(data["overall_scores"])
            ...
            print(
                f"Scoreable Runs: "
                f"{data['scoreable_runs']}"
            )

            print(
                f"Unscoreable Runs: "
                f"{data['unscoreable_runs']}"
            )
